...
export function generateSpendingByCategory(max) {
    const categories = [
        "fast food",
        "health",
        "business expenses",
...
export function generateSpendingByMerchant(max) {
...
export function generateSpendingByLocation(max, minCountries = 1) {
    const locations = Array(getRandomArbitrary(minCountries, 20))
        .fill(0)
        .map(() => Object.keys(countries)[Math.floor(Math.random() * Object.keys(countries).length)]);
    ...
    for (let i = 0; i < locations.length; i++) {
        ...
        if (i === locations.length - 1) {
            ...
            const subdivisions = [];
            // @ts-ignore
            let localFaker = countries[location];
...
export function generateSpendingByDate(max) {
    ...
    for (let i = 0; i < daysInYear; i++) {
        ...
        if (i === daysInYear - 1) {
            spendingByDate[date.toISOString().slice(0, 10).replaceAll("/", "-")] = max - totalSpent;
        }
        else {
            const amount = getRandomArbitrary(0, (max - totalSpent) / 40);
            spendingByDate[date.toISOString().slice(0, 10).replaceAll("/", "-")] = amount;
            totalSpent += amount;
        }
    }
...
export function generateTestData() {
    ...
    let individual = {
        ...
        averageReceiptUploadTime: getRandomArbitrary(60, 63072000),
